...
const HOLD_MS = 3200;
const RETRY_MS = 3500; // spacing between auto-retries — keeps us under the Gemini free-tier RPM
    ...
    return (
      <View style={[styles.fill, styles.center, styles.pad]}>
        <View style={styles.permIcon}>
          <Feather name="camera" size={26} color={TEAL} />
        </View>
        <Text style={styles.permTitle}>Camera access to read your face</Text>
        <Text style={styles.permBody}>The selfie is analysed once and never stored on our servers.</Text>
        ...
        <Pressable onPress={onManual} style={styles.linkBtn}>
          <Text style={styles.link}>Answer a few questions instead</Text>
        </Pressable>
      </View>
    );
  }
  ...
  return (
    <View style={styles.fill}>
      ...
      <View style={styles.controls}>
        ...
        <Pressable onPress={onManual} style={styles.linkBtn}>
          <Text style={[styles.link, styles.linkLight]}>Answer a few questions instead</Text>
        </Pressable>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  fill: { flex: 1, backgroundColor: '#0a1614' },
  center: { alignItems: 'center', justifyContent: 'center' },
  pad: { padding: 28, gap: 12 },
  pressed: { opacity: 0.7 },
  dim: { backgroundColor: 'rgba(6,16,14,0.45)' },
  ...
  errPill: { backgroundColor: 'rgba(15,138,126,0.95)' },
  ...
  link: { color: '#3fd9c5', fontSize: 13, fontWeight: '700' },
