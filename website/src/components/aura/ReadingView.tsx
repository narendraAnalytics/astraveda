// Sarvam is prompted (backend/app/services/aura_reading.py) to return these 7
  ...
  return (
    <div className="space-y-6">
      {sections.map((s, i) => (
        <div key={i}>
          {s.heading && (
            <h3 className="font-[family-name:var(--font-display)] text-[17px] font-medium mb-2 bg-clip-text text-transparent bg-[linear-gradient(90deg,#3b1d63,#c026d3)]">
              {s.heading}
            </h3>
          )}
          ...
          {i < sections.length - 1 && (
            <div className="mt-6 h-px bg-[linear-gradient(90deg,transparent,#7c3aed,transparent)] opacity-40" />
          )}
        </div>
      ))}
    </div>
