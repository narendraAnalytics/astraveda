...
// Auto-shutter timing (ms) — mirrors the mobile app's FaceScanner so both
// platforms feel the same: settle, then hold-steady, then the camera fires
// itself (no shutter button in a hands-free framing UI).
...
const COACH = [
  ...
  "Even light — no cap, glasses or mask",
  "Keep your head still",
];

const ANALYSING = [
  "Studying the shape of your face…",
  "Reading your forehead and brow…",
  "Following the line of your nose…",
  "Weighing the three zones…",
  "Consulting Mukha Samudrika Shastra…",
];
...
export default function FaceScanner({
  ...
  if (permission === "denied") {
    return (
      <div className="fixed inset-0 z-[200] bg-[#0a1614] flex flex-col items-center justify-center gap-4 p-8 text-center">
        <div className="w-14 h-14 rounded-[18px] bg-[#e2f4f0] flex items-center justify-center">
          <Camera size={26} color={TEAL} />
        </div>
        <h3 className="text-white text-[17px] font-semibold">Camera access to read your face</h3>
        ...
        <button
          ...
          style={{ background: `linear-gradient(135deg,${TEAL},#3fa66b)` }}
        >
  ...
  const ovalColor = holding ? GOLD : "#fff";

  return (
    <div className="fixed inset-0 z-[200] bg-[#0a1614] overflow-hidden">
      <video
        ref={videoRef}
        autoPlay
        muted
      ...
      {shotUrl && (
        <>
          <img src={shotUrl} alt="Captured face" className="absolute inset-0 w-full h-full object-cover" />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 pointer-events-none px-6">
        ...
        >
          <svg width={FRAME} height={FRAME} style={{ opacity: 0.5 + (holding ? holdProgress * 0.4 : 0) }}>
            <ellipse
              cx={FRAME / 2}
              cy={FRAME / 2}
              rx={96}
        ...
        <AnimatePresence mode="wait">
          {analysing ? (
            ...
            <div className="flex items-center gap-2 rounded-full px-4 py-2.5 max-w-[86%]" style={{ background: "rgba(15,138,126,0.94)" }}>
