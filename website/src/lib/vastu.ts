// Vastu AI API client. Mirrors frontend/src/lib/vastu.ts (the mobile app) — one
// Gemini call turns a room photo + facing direction + room type into a Vastu score,
// elemental balance, doshas and non-demolition remedies (upay). The photo is analysed once, never stored.
import { api } from "./api";
...
// Green / amber / red band for the score dial.
export function scoreBand(score: number) {
  if (score >= 75) return { color: "#3fa66b", label: "Harmonious" };
  if (score >= 50) return { color: "#e0932f", label: "Workable" };
  return { color: "#d9534f", label: "Needs attention" };
}

export const directionLabel = (key: string) => DIRECTIONS.find((d) => d.key === key)?.label ?? key;
