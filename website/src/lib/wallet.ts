// Money wallet — balance, ledger, Razorpay top-up (finalview.txt §12). Mirrors
// frontend/src/lib/wallet.ts (the mobile app) — same backend, same Neon
// `wallets` row per user, so a balance top-up on one surface shows up
// immediately on the other (no separate website wallet, just this client).
import { api } from "./api";
...
export function topupWallet(amountPaise: number, token: string | null) {
  return api<TopupCheckout>("/wallet/topup", {
    method: "POST",
...
export function pendingTopup(token: string | null) {
  return api<{ pending: null | { payment_id: string; amount_paise: number } }>(
...
export function confirmTopup(
  ...
  return api<{ balance_paise: number; credited_paise: number; bonus_paise: number }>(
