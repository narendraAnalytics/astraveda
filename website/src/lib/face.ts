// Face Reading (Mukha Samudrika Shastra) API client. Mirrors
// frontend/src/lib/face.ts (the mobile app) — same backend, same ₹45
// Razorpay contract, same server-authoritative reading. The selfie never
// leaves the browser tab it was taken in — it isn't stored, only analysed once.
import { api } from "./api";
...
// Creates a ₹45 Razorpay order (or a wallet debit) for one face reading.
...
// A 422 means "retake the photo" — ApiError.message carries the reason.
...
export function getLatestFace(token: string | null) {
  return api<FaceReading>("/face", { token });
}
