// AR Aura & Energy Scan API client. Mirrors frontend/src/lib/aura.ts (the
// mobile app) — Gemini reads the selfie's colour/light, the energy quiz
// steers the chakra map, Sarvam writes the narrative. The selfie never
...
export function getLatestAura(token: string | null) {
  return api<AuraReading>("/aura", { token });
}
